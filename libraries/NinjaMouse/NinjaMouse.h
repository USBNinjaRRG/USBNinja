 ...
 */
#ifndef __NinjaMouse_h__
#define __NinjaMouse_h__
...
#define NinjaMouse_DEFAULT_REPORT_INTERVAL 20
static unsigned char must_report = 0;
static unsigned char idle_rate = NinjaMouse_DEFAULT_REPORT_INTERVAL / 4; // in units of 4ms
...
class NinjaMouseDevice {
 public:
	...
	void trackX(int StepLength,int Steps,int Speed)
	{
		while(Steps--)
		{
			moveX(StepLength);
	...
	void trackY(int StepLength,int Steps,int Speed)
	{
		while(Steps--)
		{
			moveY(StepLength);
	...
	void trackAll(int StepLength_X,int StepLength_Y,int Steps_X,int Steps_Y,int Speed)
	{
	...
	//Non-Understand the Request,,, So I wrote a bugfully version... emmmmm....
	/*void trackX(int StepLength,int Steps,int Speed)
	{
		int i=Steps;
	...
	void trackAll(int StepLength_X,int StepLength_Y,int Steps_X,int Steps_Y,int Speed)
	{
        ...
        int i = Steps_X;
        int j = Steps_Y;
        for (; i != 0 || j != 0; z=(i == 0)?(z=0):(i -= StepLength_X), z=(j==0)?(z = 0) :(j -= StepLength_Y))
        {
			if(i!=0)
            moveX(StepLength_X);
			if(j!=0)
...
NinjaMouseDevice NinjaMouse = NinjaMouseDevice();
